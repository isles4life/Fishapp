...
2. ~~**Stripe entry fees**~~ — **SHIPPED** (backend + mobile). PaymentIntent, webhook, Apple Pay, server-side submission gate. **Still in test mode** — before charging real money: swap `pk_test_` → `pk_live_` in `mobile/App.tsx`, update `STRIPE_SECRET_KEY` + `STRIPE_WEBHOOK_SECRET` GitHub secrets to live values, re-register webhook in Stripe live dashboard.
