  ...
  return (
    <div style={{ position: 'relative', flex: 1 }}>
      {suggestions.length > 0 && mentionQuery !== null && (
        <div style={{ position: 'absolute', bottom: '100%', left: 0, right: 0, zIndex: 100, backgroundColor: C.surfaceHigh, border: `1px solid ${C.border}`, borderRadius: 8, overflow: 'hidden', marginBottom: 4, boxShadow: '0 4px 16px rgba(0,0,0,0.4)' }}>
          {suggestions.map(u => (
  ...
  return (
    <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 12, marginTop: 4 }}>
          ...
          {comments.map(c => {
            ...
            <div key={c.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 10 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 2 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <button onClick={() => myUserId && handleToggleProp(c.id)}
                    style={{ background: 'none', border: 'none', cursor: myUserId ? 'pointer' : 'default', fontSize: 13, padding: '1px 4px', opacity: c.userHasPropped ? 1 : 0.35 }}>👍</button>
                  {(c.propCount ?? 0) > 0 && (
              ...
              {editingId === c.id ? (
                ...
                <span style={{ fontSize: 13, color: C.text }}>{renderWithMentions(c.body)}</span>
              )}
              </div>
            </div>
            );
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
        <MentionInput value={body} onChange={setBody} onSubmit={doSubmit} placeholder="Add a comment…" />
        <button onClick={doSubmit} disabled={!body.trim() || submitting}
          style={{ background: C.accent, color: C.bg, border: 'none', borderRadius: 8, padding: '7px 16px', cursor: 'pointer', fontWeight: 700, fontSize: 13, opacity: (!body.trim() || submitting) ? 0.5 : 1, flexShrink: 0 }}>
          {submitting ? '…' : 'Post'}
        </button>
      </div>
      {whoCommentId && (
